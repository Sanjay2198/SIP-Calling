...
AUTO_DEMO_FALLBACK = os.getenv("AUTO_DEMO_FALLBACK", "true").strip().lower() in {"1", "true", "yes", "on"}
...
def _has_pjsua2() -> bool:
    return importlib.util.find_spec("pjsua2") is not None
...
async def make_call(request: CallRequest):
    ...
    pjsua2_available = _has_pjsua2()
    demo_mode = ALLOW_DEMO or (not pjsua2_available and AUTO_DEMO_FALLBACK)

    if not pjsua2_available and not demo_mode:
        raise HTTPException(
            status_code=503,
            detail=(
                "PJSUA2 is not installed on this server. "
                "Install PJSUA2 for real SIP calls or set ALLOW_DEMO=true "
                "(or AUTO_DEMO_FALLBACK=true) for simulation."
            ),
        )
    ...
    with _process_lock:
        ...
        if demo_mode and not pjsua2_available:
            child_env["ALLOW_DEMO"] = "true"
        try:
            if os.name == "nt":
            ...
            mode_prefix = "Demo call" if (demo_mode and not pjsua2_available) else "Call"
            return {
                "status": "success",
                "message": f"{mode_prefix} initiated to {number}. PID: {new_proc.pid}",
            }
